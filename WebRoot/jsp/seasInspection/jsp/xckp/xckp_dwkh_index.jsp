...
//删除
function deleteDwkh(dwkhID){
	if(confirm("确定删除该记录吗？")){
		document.getElementById("dwkhID").value = dwkhID;
		document.myForm.action="<%=basePath%>/patorl/patorl_deleteDwkh.do";
		document.myForm.target="_self";
		document.myForm.submit();
	}
}
...
//修改
function getUpdateDwkh(dwkhID){
	document.getElementById("dwkhID").value = dwkhID;
	document.myForm.action="<%=basePath%>/patorl/patorl_getDwkhForUpdate.do";
	document.myForm.target="_self";
	document.myForm.submit();
}
</script>
...
<form name="myForm" method="post" >
		...
		<div class="tools">
			<ul class="toolbar">
				<li>
					<a href="${contextPath}/jsp/seasInspection/jsp/xckp/xckp_dwkh_add.jsp"><span>
		...
		<table class="tablelist">
			...
			<tbody>
			<c:forEach items="${ dwkhList }" var="r" varStatus="j">
				<tr>
					<td>${j.count }</td>
					<td>${r.xclx }</td>
					<td>${r.xzqhmc }</td>
