...
SetStep.prototype.init = function(arg) {
    ...
    //console.log(this.content,w_con,w_li);
    stepsHtml.find('li').css('width', '40px').css('marginRight', w_li * 2 - 40+'px')
    ...
    if (this.opt.clickAble) {
        ...
        stepsHtml.find('li').on('click', function() {        	
            ...
            if($(this).index() <= _that.opt.index){
            	$("#ystep table").empty();
                $.each(_that.opt.arr[$(this).index()],function(key,value){
       			   	var tr = '<tr>' + 
